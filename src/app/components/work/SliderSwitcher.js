...
export default function SliderSwitcher({ openModal, post }) {
  ...
  const items = post.gallery.slice(2, 5).map((image, index) => ({ image, index }));
  ...
  return (
    <div className="relative">
      <img
        ...
        onClick={() => openModal(items[currentIndex].index + 2)}
      ...
      <button
        ...
        className="absolute top-1/2 left-1 md:left-1 lg:left-3 transform -translate-y-1/2 bg-[#AAAC24] text-white p-2 rounded-full hover:bg-[#8C9A1A] transition-all duration-200"
      >
        <FaChevronLeft className="h-4 w-3 lg:h-6 lg:w-6 md:h-5 md:w-5 md:pr-1 lg:pr-1 lg:p-1" />
      </button>
      <button
        onClick={goToNext}
        className="absolute top-1/2 right-1 md:right-1 lg:right-3 transform -translate-y-1/2 bg-[#AAAC24] text-white p-2 rounded-full hover:bg-[#8C9A1A] transition-all duration-200"
      >
